...
(function($){	
	var _jQuizMeLayout = $("<div/>").addClass( "quiz-el").append( 
		$("<div/>").addClass( "q-header q-innerArea blue lighten-3").append(
			$("<div/>").addClass( "q-counter"),
			$("<div/>").addClass( "q-title")
		),
		$("<div/>").addClass( "q-help q-innerArea  blue lighten-3").append( 
			$("<div/>").addClass( "q-help-menu" ).append(
				$( "<span/>" ).addClass( "q-quit-area" ).append(
					$( "<input type ='button'/>" ).addClass( "q-quit-btn btn blue darken-2" ),
					$( "<input type='button'/>" ).addClass( "q-quitYes-btn q-quit-confirm btn blue darken-2" ).hide(),
					$( "<input type='button'/>" ).addClass( "q-quitNo-btn q-quit-confirm btn blue darken-2" ).hide()
				),
				$("<input type='button'/>").addClass( "q-help-btn btn blue darken-2" ),									
				$( "<span/>" ).addClass( "q-timer-area" ),
				$("<div/>").addClass( "q-help-info" ).hide()
			)
		),
		$("<div/>").addClass( "q-review-menu q-innerArea  blue lighten-3").append( 
			$("<input type='button'/>").addClass( "q-details-btn q-reviewBar-btns btn blue darken-2" ),
			$("<input type='button'/>").addClass( "q-review-btn q-reviewBar-btns btn blue darken-2" ),
			$("<div/>").addClass( "q-reviewBar q-innerArea  blue lighten-3").append(
				$("<input type='button'/>").attr({ "class": "q-leftArrow q-review-arrows btn blue darken-2", "value": "<<" }),
				$("<input type='button'/>").attr({ "class": "q-rightArrow q-review-arrows btn blue darken-2", "value": ">>" }),
				$( "<span/>" ).addClass( "q-review-nav" ).append(
					$("<select/>").addClass( "q-review-index-all q-review-index browser-default"),
		...
		$("<div/>").addClass( "q-intro q-innerArea  blue lighten-3" ).append(
			$( '<p/>' ).addClass( "q-intro-info" ),
			$( '<input type="button"/>' ).addClass( "q-begin-btn btn blue darken-2" )
		).hide(),
		$("<div/>").addClass( "q-prob q-innerArea  blue lighten-3" ).append( 
			$("<div/>").addClass( "q-ques q-probArea"),
			$("<div/>").addClass( "q-ans q-probArea").append(
					$( "<div/>" ).addClass( "q-ansSel" ),
					$( "<input type='button'/>" ).addClass( "q-check-btn btn blue darken-2" ),
					$( "<input type='button'/>" ).addClass( "q-next-btn btn blue darken-2" )
				),
			$("<div/>").addClass( "q-result q-probArea" ).hide() 
		),
		$("<div/>").addClass( "q-gameOver q-innerArea  blue lighten-3" ).append(
			...
			$("<div/>").addClass( "q-options q-probArea q-center").append( 
				$("<input type='button'/>").addClass( "q-restart-btn btn blue darken-2" ),
				$("<input type='button'/>").addClass( "q-del-btn btn blue darken-2" ) 
			)
		).hide()
	);
	...
	_lang = {
			ans:{	// _lang.ans are shown during the display of .q-result, the answer result.
				corrAns: "Jawabanmu salah, yang benar adalah :",
				praise: 'Jawaban kamu benar!',
			...
			btn:{	// [ "text", "title" ]
				begin: [ "Mulai Quiz" ],
				check: [ "Cek", "Cek Jawabanmu" ],
				del: [ "Delete", "Delete quiz" ],
				help: [ "Help", 'Click for help'],
				next: [ "Berikut", "Pertanyaan Berikutnya" ],
				restart: [ "Ulangi", "Restart the quit over" ],
				details: [ "Detail", "View score report" ],
				review: [ "Review", "Review Questions" ],
				showOnlyMissed: [ " *", "Click to show only missed questions." ],
				quit: [ "Berhenti", "Quit quiz" ],
				quitNo: [ "Batal", "Go Back" ],
				quitYes: [ 'Ya, berhenti', "Yes, quit" ]
			},
			err:{
			...
			buildQuizType = {
				...
				'multipleChoice' : function( iProp, olVer ){
					...
					// ansSel = <select>[ <option> is the answer + <option> * settings.multiLen]
						var d;
						if( olVer ){
							d = $( '<div><ol class="q-ol collection"></ol></div>' );
							$( d ).one( "click", function(){
								releaseButtonBlock();
							});
						}
						else{
							d = $( '<div><select class="q-select browser-default"></select></div>' );
					...
					while (numOfQues--) {
						...
						if (hasAnsSel(iProp, numOfQues)) {
							...
							while (len--) {
								...
								x = (olVer) ? ["<li class = 'q-ol-li collection-item'><input type='radio' class='q-radioBtn'/><span class='q-ol-li-ansSel'>", ansSelChoice, "</span></li> "].join("") : ["<option>", ansSelChoice, "</option> "].join("");
								optHtml = x + optHtml;
							}
						} else {
